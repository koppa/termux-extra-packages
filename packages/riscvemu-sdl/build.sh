...
TERMUX_PKG_DESCRIPTION="RISC-V system emulator (with framebuffer enabled)"
...
termux_step_make() {
    if [ "${TERMUX_ARCH}" = "aarch64" ] || [ "${TERMUX_ARCH}" = "x86_64" ]; then
        make -j ${TERMUX_MAKE_PROCESSES} CC="${CC}" CONFIG_SDL=y CONFIG_INT128=y
    else
        make -j ${TERMUX_MAKE_PROCESSES} CC="${CC}" CONFIG_SDL=y
    fi
}
